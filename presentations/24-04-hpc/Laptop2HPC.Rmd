...
Interactive use _must_ still be mediated by the batch system to fairly share the resources. 

This ensures that the computation takes place on a compute nodes.

Installing R packages must _not_ be done on the login nodes. Getting in early ;-)
...
So, yes, you can run RStudio. Indeed,  _two_ ways!
...
- Be careful _not_ to reset your `.libPaths()` and ignore these.
...
It is often more effective to break it into smaller pieces and use a _high throughput computing (HTC)_ approach. Simplest would look like
...
_The possibilities that arise from combining R with HPC are well worth the learning curve!_
